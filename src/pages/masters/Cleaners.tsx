...
export const Cleaners: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', mobile: '', employeeId: '', aadharNumber: '', status: 'active' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', mobile: '', employeeId: '', aadharNumber: '', status: 'active' });
  };
    ...
    <div className="space-y-6">
            ...
            <tbody className="divide-y">
              {filtered.map(cleaner => (
                <tr key={cleaner.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">{cleaner.aadharNumber || 'N/A'}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button onClick={() => { setSelectedCleaner(cleaner); setFormData({ name: cleaner.name, mobile: cleaner.mobile, employeeId: cleaner.employeeId, aadharNumber: cleaner.aadharNumber || '', status: cleaner.status }); setIsModalOpen(true); }} className="p-2 text-gray-400 hover:text-blue-600"><Edit2 size={16} /></button>
      ...
      <BaseModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedCleaner ? 'Edit Cleaner' : 'Add New Cleaner'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input required placeholder="Aadhar Number" className="w-full p-2 border rounded" value={formData.aadharNumber} onChange={e => setFormData({...formData, aadharNumber: e.target.value})} />
