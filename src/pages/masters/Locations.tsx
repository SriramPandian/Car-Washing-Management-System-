  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button icon={<Plus size={18} />} onClick={() => { setSelectedLoc(null); setFormData({ name: '', address: '', status: 'active' }); setIsModalOpen(true); }}>Add Location</Button>
      </div>
                ...
                <tr key={loc.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button onClick={() => { setSelectedLoc(loc); setFormData({ name: loc.name, address: loc.address, status: loc.status }); setIsModalOpen(true); }} className="p-2 text-gray-400 hover:text-blue-600"><Edit2 size={16} /></button>
      ...
      <BaseModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedLoc ? 'Edit Location' : 'Add Location'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <textarea required placeholder="Address" className="w-full p-2 border rounded" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
